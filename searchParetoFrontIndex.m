...
%% Returen the index of the input struct array 

function ParetoFrontIndex = searchParetoFrontIndex(Data)
%% Initialize the dataset
nFun        = length(Data.Fun);
PopSize     = numel(Data.Fun(1).Value);
nValue      = PopSize;
...
[~,IndexSortTemp] = sortrows(ValueMatrix);
IndexSort   = IndexArray(IndexSortTemp);

%% Return the result
ParetoFrontIndex = IndexSort;
end
